    ...
    <div class="mt-[30px]">
        <form class="mx-auto" @submit.prevent="registerPopup = true" @close-modal="registerPopup = false">
            <div class="relative mb-4">
                ...
                <img src="../icons/User.svg" alt="Mail Icon" class="absolute left-3 top-2.5 h-5 w-5">
            </div>
            <div class="relative mb-4">
                ...
                <p v-if="isInvalidEmail" class="text-red-500 text-sm mt-1">{{ emailErrorMessage }}</p>
            </div>
            <div class="mb-4 relative">
                <input id="password" v-model="password" :type="showPassword ? 'text' : 'password'"
                    :class="['block px-12 py-2 w-full text-gray-900 bg-transparent rounded-lg border border-solid', isInvalidPassword ? 'border-red-500' : 'border-gray-300', 'appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer']"
                ...
                <img src="../icons/ShowPassword.svg" alt="Show Password Icon"
                    class="absolute right-3 top-2.5 h-5 w-5 cursor-pointer" @click="showPassword = !showPassword" />
                <p v-if="password && isInvalidPassword" class="text-red-500 text-sm mt-1">The password must consist of at
                    least 8
                    characters.</p>
            </div>
            <div class="mb-5 relative">
                <input id="password" v-model="confirmPassword" :type="showConfirmPassword ? 'text' : 'password'"
                    :class="['block px-12 py-2 w-full text-gray-900 bg-transparent rounded-lg border border-solid', isInvalidConfirmPassword ? 'border-red-500' : 'border-gray-300', 'appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer']"
                ...
                <p v-if="confirmPasswordTouched && confirmPassword !== password" class="text-red-500 text-sm mt-1">The
                    password you entered does not match.</p>
            </div>
            <div class="mb-5">
                <button
                    type="submit"
        ...
        <div class="text-center text-sm">
            <p class="text-[#333333]">Already have an account?
